...
- **Pages**: HomePage (`/`), ResourcesHub (`/resources`), ContentPage (`/resources/:slug`), StartPage (`/start` — private signup, NOT linked publicly; `noindex,nofollow` is rendered both into the static prerender shell by `scripts/prerender.ts` and at runtime via react-helmet-async)
