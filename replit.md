...
- **Pages**: HomePage (`/`), ResourcesHub (`/resources`), ContentPage (`/resources/:slug`), ProblemPage (`/problem` — public marketing dashboard cataloging real-world attorney-client privilege violations from court filings, transcripts, news, and leaked LLM chats; data lives in `src/data/privilegeIncidents.ts`; filterable by source category / year / free-text; fully prerendered + sitemap-listed; tip submissions go to `mailto:tips@sntlabs.io` or the existing demo modal), StartPage (`/start` — private signup, NOT linked publicly; `noindex,nofollow` is rendered both into the static prerender shell by `scripts/prerender.ts` and at runtime via react-helmet-async)
- **Adding a new /problem incident**: User drops a source URL or court PDF in chat. Agent runs `pnpm --filter @workspace/sentinel-counsel run new:incident -- <url>` to print a starter object, fills the TODO fields from the source (verbatim quote, attribution, headline, why-it-matters), pastes it at the TOP of the `privilegeIncidents` array in `src/data/privilegeIncidents.ts`, and **MUST verify `sourceUrl` resolves via web search before committing — fabricated/dead links are not acceptable**. Auto-publishes on next deploy; counter, year filter, sitemap, and JSON-LD all derive from the array.
- **Private signup `/start`**: Three-step intake → review → Stripe Payment Element. POST `/api/intake` returns `{ submissionId, submissionToken }`; the opaque token is required on POST `/api/create-subscription` (timing-safe compare, identical 404 for missing-row vs bad-token to prevent enumeration). Subscription creation runs inside a Postgres `SELECT … FOR UPDATE` transaction with Stripe `idempotencyKey`s, so concurrent calls are serialized and Stripe never duplicates customers/subscriptions. On `invoice.paid`/`invoice.payment_succeeded`, the webhook handler (`stripe-webhook-handler.ts`) atomically claims `team_email_sent_at` / `client_email_sent_at` via `UPDATE … WHERE col IS NULL RETURNING`, releases the claim back to NULL on email failure, and only flips the `emails_sent_at` roll-up once both per-email markers are set. Pricing: $2,500/license/yr, min 10 licenses, quarterly billing, 1yr or 2yr (10% off). Webhook is registered before `express.json()` and uses `express.raw({ type: "application/json" })`.
